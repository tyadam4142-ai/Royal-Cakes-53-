...
async function load(){
 if(!requireSB())return;
 const [c,m,h,p,s,d]=await Promise.all([
...
async function fileToUrl(file){
 ...
 const {error}=await sb.storage.from("royal-cakes").upload(path,file,{upsert:false,cacheControl:"31536000"});
 if(error)throw error;
 return sb.storage.from("royal-cakes").getPublicUrl(path).data.publicUrl;
}
async function uploadMedia(files, placement){
 if(!requireSB()||!files.length)return;
...
function renderHero(){
 ...
 document.querySelectorAll("[data-save-hero]").forEach(b=>b.onclick=async()=>{const id=b.dataset.saveHero;const obj={};document.querySelectorAll(`[data-h="${id}"]`).forEach(x=>obj[x.dataset.f]=x.type==="number"?+x.value:x.value);await sb.from("hero_slides").update(obj).eq("id",id);toast("Homepage slide saved");load()});
 document.querySelectorAll("[data-del-hero]").forEach(b=>b.onclick=async()=>{await sb.from("hero_slides").delete().eq("id",b.dataset.delHero);toast("Slide removed");load()});
}
$("#newProduct").onclick=async()=>{if(!requireSB())return;const {error}=await sb.from("products").insert({name:"New Cake",description:"",category:"Cakes",price:0,available:true,featured:false,sort_order:cache.products.length});if(error)toast(error.message);else load()};
...
function renderProducts(){
 ...
 document.querySelectorAll("[data-save-product]").forEach(b=>b.onclick=async()=>{const id=b.dataset.saveProduct,obj={};document.querySelectorAll(`[data-p="${id}"]`).forEach(x=>obj[x.dataset.f]=x.type==="number"?+x.value:x.value);await sb.from("products").update(obj).eq("id",id);toast("Product published");load()});
 document.querySelectorAll("[data-del-product]").forEach(b=>b.onclick=async()=>{if(confirm("Delete this product?")){await sb.from("products").delete().eq("id",b.dataset.delProduct);load()}});
}
...
function renderStories(){
 ...
 document.querySelectorAll("[data-save-story]").forEach(b=>b.onclick=async()=>{const id=b.dataset.saveStory,obj={};document.querySelectorAll(`[data-s="${id}"]`).forEach(x=>obj[x.dataset.f]=x.value);await sb.from("stories").update(obj).eq("id",id);toast("Story published");load()});
 document.querySelectorAll("[data-del-story]").forEach(b=>b.onclick=async()=>{await sb.from("stories").delete().eq("id",b.dataset.delStory);load()});
}
...
$("#saveSettings").onclick=async()=>{const row={bakery_name:$("#setName").value,tagline:$("#setTagline").value,whatsapp_number:$("#setWhatsApp").value,logo_url:$("#logoSelect").value||null,updated_at:new Date().toISOString()};if(cache.config.id)await sb.from("site_config").update(row).eq("id",cache.config.id);else await sb.from("site_config").insert(row);toast("Settings published");load()};
